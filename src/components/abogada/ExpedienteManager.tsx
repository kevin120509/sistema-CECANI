...
const getHitoTemplates = (hitoNombre: string, empresa: string, abogada: string, fecha: string, hora?: string) => {
  ...
  const templates: Record<string, { tipo: string, titulo: string, mensaje: string, sugerencias: string[] }> = {
    ...
    'Inscripción SAT': {
      ...
      sugerencias: ['TESTIMONIO NOTARIAL', 'IDENTIFICACIÓN VIGENTE', 'COMPROBANTE DE DOMICILIO DE LA AC']
    }
  };
...
const DOCS_PERSONALES = [
  'INE FRENTE', 'INE REVERSO', 'CURP ACTUALIZADA', 'COMPROBANTE DOMICILIO', 
  'CONSTANCIA SITUACIÓN FISCAL', 'E.FIRMA (.CER / .KEY)', 'PAGO INICIAL'
];

const DOCS_PROCESO = [
  'PROYECTO DE ACTA', 'TESTIMONIO NOTARIAL', 'COPIA CERTIFICADA', 
  'ACUSE CITA SAT', 'RFC MORAL'
];
      ...
      <div className="flex items-center gap-2 shrink-0">
        {url ? (
          <div className="flex gap-1.5 md:gap-2">
            ...
            {isPending && (
              <div className="p-2 md:p-2.5 bg-sky-900/30 text-sky-400 rounded-xl shadow-lg cursor-help border border-sky-800" title={`Motivo enviado: ${motivo_borrado || 'No especificado'}`}>
...
export default function ExpedienteManager({ expedientes, hitos, alertasHoy, solicitarAltaPanel }: ExpedienteManagerProps) {
  ...
  // Estados para Documentos Extras
  const [documentosExtrasDisponibles, setDocumentosExtrasDisponibles] = useState<string[]>([]);
    ...
    return (
      <div className="flex min-h-screen bg-slate-950 text-slate-300 font-sans overflow-x-hidden">
        <aside className="w-72 shrink-0 bg-slate-900 text-slate-300 flex flex-col min-h-screen sticky top-0 border-r border-slate-800">
          <div className="p-6 border-b border-slate-800 flex items-center justify-end">
        ...
        <main className="flex-1 p-6 md:p-8 w-full max-w-[1600px] mx-auto">
          ...
          {/* Overview header removed as requested */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
             ...
             <div className="bg-slate-900 rounded-2xl p-6 relative overflow-hidden shadow-sm border border-slate-800">
               <div className="flex justify-between items-start mb-4"><span className="px-2.5 py-1 rounded bg-red-600/20 text-red-400 text-xs font-bold flex items-center gap-1">⚠ Hoy</span></div>
               <p className="text-slate-400 text-sm font-medium">Alertas del Día</p>
               <div className="flex items-end justify-between mt-1"><h3 className="text-3xl font-bold text-white">{alertasHoy.length}</h3><AlertTriangle size={32} className="text-red-600/80" /></div>
             </div>
             <div className="bg-slate-900 rounded-2xl p-6 relative overflow-hidden shadow-sm border border-slate-800">
               <div className="flex justify-between items-start mb-4"><span className="px-2.5 py-1 rounded bg-red-600/20 text-red-400 text-xs font-bold flex items-center gap-1">↓ Vencidos</span></div>
               <p className="text-slate-400 text-sm font-medium">Recordatorios Pendientes</p>
               <div className="flex items-end justify-between mt-1"><h3 className="text-3xl font-bold text-white">{recordatoriosPendientes.length}</h3><Calendar size={32} className="text-red-600/80" /></div>
             </div>
          </div>
          )}
          ...
          {dashTab === 'clientes' && (
            <div className="space-y-4">
               {filteredExpedientes.length === 0 ? (
                  <div className="bg-slate-900 rounded-2xl border border-slate-800 p-16 text-center shadow-sm"><Users size={48} className="mx-auto text-slate-600 mb-4" /><p className="text-slate-400 font-medium text-sm">Sin clientes asignados</p></div>
               ) : (
                      ...
                      return (
                        <div key={exp.id} className={`bg-slate-900 rounded-2xl border p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-colors shadow-sm ${hasAlert ? 'border-red-500/50 hover:border-red-500' : 'border-slate-800 hover:border-slate-700'}`}>
                          ...
                          <div className="flex-1 px-4 max-w-xs w-full">
                            <div className="flex justify-between text-[10px] font-bold uppercase text-slate-400 mb-2"><span>Fase {completadosExp}/{totalExp}</span><span>{Math.round((completadosExp / totalExp) * 100)}%</span></div>
          ...
          {dashTab === 'agenda' && (
            <div className="space-y-6">
              <div className="flex bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-sm max-w-sm">
                <button onClick={() => setAgendaView('lista')} className={`flex-1 px-4 py-2 text-xs font-bold transition-all flex items-center justify-center gap-2 ${agendaView === 'lista' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/50'}`}><ClipboardList size={14}/> Lista</button>
                <button onClick={() => setAgendaView('calendario')} className={`flex-1 px-4 py-2 text-xs font-bold transition-all flex items-center justify-center gap-2 ${agendaView === 'calendario' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/50'}`}><Calendar size={14}/> Calendario</button>
              </div>
              {agendaView === 'lista' && (
                <div className="space-y-6">
  ...
  const contrato = (selectedExpediente as any).contratos?.[0] || (selectedExpediente as any).contrato;
  const isPdf = (url: string | undefined | null) => url?.toLowerCase().endsWith('.pdf');
  
  const urlContratoDoble = isPdf(contrato?.url_pdf_doble_firma) ? contrato?.url_pdf_doble_firma : null;
  const urlContratoGenerado = isPdf(contrato?.url_pdf_generado) ? contrato?.url_pdf_generado : null;
  const urlContratoCliente = isPdf(contrato?.url_pdf_firmado_cliente) ? contrato?.url_pdf_firmado_cliente : null;
    ...
    <div className="flex min-h-screen bg-slate-950 text-slate-300 font-sans overflow-x-hidden p-6 md:p-8">
      <div className="max-w-[1600px] w-full mx-auto space-y-6 md:space-y-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <button onClick={closeDetail} className="flex items-center gap-2 text-slate-400 hover:text-white font-bold text-sm transition-all bg-slate-900 px-4 py-2 rounded-xl shadow-sm border border-slate-800">
            ← Volver al Listado
          </button>
          <div className="flex items-center gap-4 w-full md:w-auto">
           ...
           <div className="flex items-center gap-3 flex-1 md:flex-initial justify-end">
               <span className="text-xs font-bold text-slate-500 uppercase">Nº Control:</span>
               <input type="text" defaultValue={(selectedExpediente as any).numero_control || ''} onBlur={e => handleUpdateControl(e.target.value)} className="bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-sm font-bold text-slate-200 outline-none focus:border-sky-600 w-full md:w-48 shadow-sm transition-all" />
             </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-3xl p-8 text-white flex flex-col lg:flex-row justify-between gap-8 items-center shadow-xl border border-slate-800 relative overflow-hidden">
          <div className="space-y-3 relative z-10 text-center lg:text-left">
            <h1 className="text-3xl font-bold tracking-tight">{selectedExpediente.nombre_empresa}</h1>
            <div className="flex flex-col lg:flex-row items-center gap-2 lg:gap-4 justify-center lg:justify-start">
              <span className="bg-slate-800 text-slate-300 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider border border-slate-700">
                {(selectedExpediente as any).figura?.descripcion || 'Sin figura legal'}
              </span>
              <p className="text-slate-400 text-sm font-medium flex items-center gap-1.5">
                <User size={16}/> {(selectedExpediente as any).cliente?.nombre_completo || 'Sin cliente titular'}
              </p>
            </div>
          </div>
        ...
        <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-800 p-2 md:p-3 overflow-x-auto custom-scrollbar">
          <div className="flex items-center gap-2 min-w-max">
            {[
              { id: 'etapa_legal', label: '1. Etapa Legal', icon: <ClipboardList size={16}/> },
              { id: 'documentacion', label: '2. CheckList Docs', icon: <BookOpen size={16}/> },
        ...
        <div className="bg-slate-900 rounded-3xl shadow-xl border border-slate-800 min-h-[600px] overflow-hidden">
          {activeTab === 'etapa_legal' && (
            <div className="p-8 space-y-8 bg-slate-900">
                 <div className="flex items-center justify-between"><h3 className="text-xl font-bold text-slate-200 flex items-center gap-3"><FileText size={24} className="text-red-400"/> Concentrado de Datos</h3><button onClick={handleSaveConcentrado} disabled={isSavingConcentrado} className="px-6 py-2 bg-red-600 text-white font-bold text-xs rounded-xl shadow-lg shadow-red-600/20 hover:bg-red-500 transition-all flex items-center gap-2 disabled:opacity-50">{isSavingConcentrado ? <Loader2 size={16} className="animate-spin"/> : 'Guardar Cambios'}</button></div>
                 ...
                 <div className="space-y-8 max-h-[600px] overflow-y-auto custom-scrollbar pr-2">
                   {[
                      ...
                      <div key={seccion.titulo} className="bg-slate-950/30 p-6 rounded-2xl border border-slate-800 space-y-4 shadow-sm">
                        <h4 className="text-sm font-black uppercase tracking-widest text-slate-400 border-b border-slate-800 pb-2">{seccion.titulo}</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                           {seccion.campos.map(campo => (
                             <div key={campo} className="space-y-1.5"><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{campo.replace(/_/g, ' ')}</label>{campo === 'objeto_social_ventas' ? (<textarea value={concentradoForm[campo] || ''} onChange={e => handleConcentradoChange(campo, e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-sm text-slate-200 min-h-[120px] outline-none focus:border-sky-600" />) : (<input type="text" value={concentradoForm[campo] || ''} onChange={e => handleConcentradoChange(campo, e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-sm text-slate-200 outline-none focus:border-sky-600" />)}</div>
                           ))}
                        </div>
                      </div>
                   ))}
          ...
          {activeTab === 'documentacion' && (
            <div className="p-6 md:p-8 space-y-8 bg-slate-950 min-h-full">
              {/* SECCIÓN DE INTEGRANTES MOVIDA AQUÍ */}
              <div className="bg-slate-900 p-6 md:p-8 rounded-3xl border border-slate-800 shadow-xl space-y-6">
                <div className="flex items-center justify-between">
                   <h3 className="text-xl font-bold text-slate-200 flex items-center gap-3"><Users size={24} className="text-sky-400"/> Integrantes de Firma</h3>
                   <button onClick={() => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {integrantes.map((int: any) => (
                    <div key={int.id} className="group bg-slate-950/50 p-5 rounded-2xl border border-slate-800 flex items-center justify-between shadow-sm transition-all hover:border-sky-600/30">
                      <div className="flex items-center gap-3"><div className="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-400"><User size={20}/></div><p className="font-bold text-sm text-slate-300 uppercase">{int.nombre_completo}</p></div>
                      <button onClick={() => handleDeleteIntegrante(int.id)} className="p-2 text-slate-500 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100" title="Eliminar Integrante"><Trash2 size={18}/></button>
                    </div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 {(() => {
                   ...
                   const modelPersonal = DOCS_PERSONALES.map(t => {
                     ...
                     return { type: t, label: t, url: found?.url_archivo, docId: found?.id, validado: found?.validado, motivo_rechazo: found?.motivo_rechazo, solicitud_borrado: found?.solicitud_borrado, motivo_borrado: found?.motivo_borrado, estatus_borrado: found?.estatus_borrado };
                   });
                 ...
                 {integrantes.map((int: any) => {
                   ...
                   const model = DOCS_CATALOGO.filter(t => !['PAGO INICIAL'].includes(t)).map(t => {
                     const dbType = DOCS_MAP[t] || t;
                 ...
                 {documentosExtrasDisponibles.length > 0 && (() => {
                   ...
                   const modelExtras = documentosExtrasDisponibles.map(t => {
                     const dbType = t; // Use the custom name as type
                     const found = docsGenerales.find(d => d.tipo === dbType);
                     return { type: t, label: t, url: found?.url_archivo, docId: found?.id, validado: found?.validado, motivo_rechazo: found?.motivo_rechazo, solicitud_borrado: found?.solicitud_borrado, motivo_borrado: found?.motivo_borrado, estatus_borrado: found?.estatus_borrado };
                   });
               ...
               <div className="space-y-6">
                 ...
                 <div className="space-y-3">
                   {hitosLegales.map((hito, idx) => {
                     ...
                     // Lógica de colores según recordatorio (Urgencia)
                     ...
                     return (
                       <div key={hito.id} className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${cardBg}`}>
                         <div className="relative">
                           {isProcessing ? <Loader2 size={24} className="animate-spin text-slate-400" /> : 
                           <button onClick={(e) => { e.stopPropagation(); handleToggleHito(hito.id.toString(), !isCompleted); }} className="focus:outline-none transition-transform hover:scale-110">
                       ...
                       <div key={r.id} className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex justify-between gap-4 shadow-sm relative overflow-hidden group">
                         ...
                         <div className="flex flex-col items-end gap-2">
                            <div className="flex gap-1">
                              {r.estatus === 'pendiente' && (<button onClick={() => { setSelectedReminderId(r.id); setShowReminderForm(r.titulo); }} className="px-3 py-1.5 text-[10px] font-bold text-slate-300 bg-slate-800 rounded hover:bg-slate-700 transition-colors border border-slate-700">Completar</button>)}
                              <button onClick={() => handleDeleteReminder(r.id)} className="p-1.5 text-slate-500 hover:text-red-500 transition-colors" title="Eliminar Recordatorio"><Trash2 size={14}/></button>
                            </div>
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${r.estatus === 'completado' ? 'bg-green-600/20 text-green-400' : r.fecha < hoy ? 'bg-red-600/20 text-red-400' : 'bg-yellow-500/20 text-yellow-500'}`}>{r.estatus}</span>
                         </div>
                       </div>
                     ))
                   )}
        ...
        {showReminderForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-slate-900 rounded-[3rem] shadow-2xl max-w-6xl w-full p-8 md:p-12 border-4 border-slate-800 max-h-[90vh] overflow-y-auto custom-scrollbar">
               ...
               {(() => {
                 const hitoEncontrado = hitos.find(h => h.nombre === showReminderForm);
                 if (!hitoEncontrado) return null;
                 return (<ReminderForm hito={hitoEncontrado} expediente={selectedExpediente} onSuccess={() => setShowReminderForm(null)} />);
               })()}
            </motion.div>
          </div>
        )}
...
function ReminderForm({ hito, expediente, onSuccess }: { hito: CatalogoHito, expediente: ExpedienteAbogada, onSuccess: () => void }) {
  ...
  const generatedMessage = useMemo(() => {
    ...
    if (selectedDocs.length > 0) msg += `Para continuar con el proceso y aprovechar nuestra cita, es indispensable que nos envíe o tenga listos los siguientes documentos:\n\n- ${selectedDocs.join('\n- ').toUpperCase()}`;
    return msg;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await crearRecordatorio({ expediente_id: expediente.id, tipo: template.tipo as any, titulo: template.titulo, descripcion: generatedMessage, fecha: fecha, hora: hora || undefined, docs_requeridos: selectedDocs, notificar_abogada: true, notificar_cliente_whatsapp: false });
    if (res.success) {
...
function ConcentradoCard({ title, children, color, className = "" }: any) {
  ...
  return (
    <div className={`p-6 md:p-8 rounded-3xl border-2 shadow-lg space-y-6 transition-all hover:shadow-xl ${colors[color] || colors.slate} ${className}`}>
      <h3 className="text-xs md:text-sm font-black uppercase tracking-widest border-b-2 border-current/10 pb-4 text-center">{title}</h3>
...
function RecordatorioCard({ r, color, onClick }: { r: any; color: 'red' | 'sky'; onClick: () => void; }) {
  const colors = { 
    red: { bg: 'bg-rose-950/20', border: 'border-rose-900/50', text: 'text-rose-400', badge: 'bg-rose-900/30 text-rose-400' }, 
    sky: { bg: 'bg-sky-950/20', border: 'border-sky-900/50', text: 'text-sky-400', badge: 'bg-sky-900/30 text-sky-400' } 
  };
  const c = colors[color] || colors.sky;
  return (
    <div onClick={onClick} className={`${c.bg} border-2 ${c.border} rounded-2xl px-6 py-4 flex items-center justify-between gap-4 cursor-pointer hover:shadow-md transition-all`}>
      <div className="flex items-center gap-4 min-w-0">
        <div className={`w-2 h-10 rounded-full ${c.badge.split(' ')[0]}`}/>
        <div className="min-w-0">
          <p className="text-[11px] font-black text-slate-200 uppercase truncate">{r.empresa}</p>
      ...
      <div className="shrink-0 text-right space-y-1">
        ...
        {r.hora && (<p className="text-[9px] font-bold text-slate-400 uppercase">{r.hora}</p>)}
