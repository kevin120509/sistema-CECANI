  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-24 py-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-6">
        <div className="w-20 h-20 bg-rose-500/10 text-rose-500 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-2xl border border-rose-500/20">
          <ShieldAlert size={36} />
        </div>
      ...
      <div className="bg-slate-900 rounded-3xl p-8 md:p-16 shadow-2xl border border-slate-800 relative overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-slate-950/95 backdrop-blur-md flex flex-col items-center justify-center p-10 text-center">
              <div className="relative w-24 h-24 mb-8">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <RotateCcw className="text-sky-600" size={32} />
                </div>
              </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {hayAlgunRechazado && (
            <footer className="pt-12 border-t border-slate-800 flex justify-end">
              <button
                type="submit"
                disabled={isPending}
...
function DocRow({ label, doc, rechazado, archivo, isPending, onFileChange, onClear }: {
  ...
  if (!rechazado) {
    return (
      <div className="flex items-center justify-between p-8 bg-slate-950/40 border border-slate-800 rounded-3xl transition-all duration-500 hover:bg-slate-950/60">
  ...
  return (
    <div className="rounded-3xl border border-rose-500/30 bg-rose-500/5 overflow-hidden transition-all duration-500 hover:bg-rose-500/[0.07]">
