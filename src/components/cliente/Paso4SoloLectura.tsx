...
const ESTATUS_CONFIG: Record<string, { label: string, color: string, icon: any, desc: string, glow: string }> = {
  revision_directora: {
    ...
    color: 'border-blue-500/20 bg-[#0a0f1d]/80 text-blue-400',
  ...
  en_proceso: {
    ...
    color: 'border-amber-500/20 bg-[#0a0f1d]/80 text-amber-400',
  ...
  completado: {
    ...
    color: 'border-emerald-500/20 bg-[#0a0f1d]/80 text-emerald-400',
  ...
  rechazado: {
    ...
    color: 'border-rose-500/20 bg-[#0a0f1d]/80 text-rose-400',
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 pb-24 py-4">
      ...
      <motion.div 
        ...
        className={`rounded-[3.5rem] border-2 p-10 md:p-14 shadow-2xl relative overflow-hidden premium-border ${config.color} ${config.glow}`}
      >
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
          <div className="w-24 h-24 rounded-[2rem] bg-[#030712] border border-white/5 flex items-center justify-center shadow-2xl shrink-0 group">
      ...
      {pendienteAsignar && (
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-blue-500/10 rounded-[2.5rem] p-8 text-white flex items-center gap-8 shadow-2xl border border-blue-500/20 premium-border overflow-hidden relative"
        >
          <div className="w-16 h-16 bg-blue-500 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(59,130,246,0.4)] shrink-0 animate-pulse relative z-10">
            <UserPlus size={28} className="text-white" />
          </div>
          <div className="relative z-10">
            <h4 className="text-sm font-black uppercase tracking-[0.3em] text-blue-400 mb-1">Vinculación en Trámite</h4>
          ...
          <Sparkles className="absolute right-10 top-1/2 -translate-y-1/2 text-blue-500/20" size={64} />
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-4 space-y-10">
          <section className="bg-[#0a0f1d] rounded-[3rem] p-10 shadow-2xl border border-white/5 premium-border">
            <header className="flex items-center gap-4 mb-10 border-b border-white/5 pb-8">
              <div className="w-10 h-10 bg-white/5 text-blue-400 rounded-xl flex items-center justify-center border border-white/5 shadow-lg"><Building2 size={20} /></div>
              <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-500">Expediente</h3>
            </header>
            
            <div className="space-y-8">
              <div>
                <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] mb-3">Razón Social</p>
          ...
          {contrato?.url_pdf_generado && (
            <section className="bg-[#050811] rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group border border-blue-500/10">
              <header className="flex items-center gap-4 mb-10 relative z-10">
                <div className="w-10 h-10 bg-blue-500 text-white rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.4)]"><Download size={20} /></div>
                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-400">Downloads</h3>
              </header>
        ...
        <div className="lg:col-span-8">
          <div className="bg-[#0a0f1d]/60 backdrop-blur-3xl rounded-[4rem] p-8 md:p-16 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] border border-white/5 premium-border">
            <header className="flex items-center gap-6 border-b border-white/5 pb-10 mb-10">
              <div className="w-14 h-14 bg-[#030712] text-blue-500 rounded-2xl flex items-center justify-center shadow-inner border border-white/5"><FileText size={28} /></div>
              <div>
                <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-blue-500 mb-2">Archivo Digital</h4>
            ...
            <div className="grid grid-cols-1 gap-6">
              {documentos.length === 0 ? (
                <div className="text-center py-24 bg-[#030712]/50 rounded-[3rem] border-2 border-dashed border-white/5">
                ...
                documentos.map((doc, idx) => (
                  <motion.div
                    ...
                    className="flex items-center justify-between p-8 rounded-[2.5rem] bg-[#030712]/40 border border-white/5 hover:border-blue-500/30 hover:shadow-2xl transition-all duration-500 group premium-border"
                  >
                    <div className="flex items-center gap-6">
                      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-500 group-hover:scale-110 shadow-lg border ${doc.validado ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>
                    ...
                    <div className="flex items-center gap-6">
                      <div className={`hidden sm:flex items-center gap-2 px-5 py-2 rounded-full border text-[9px] font-black uppercase tracking-widest shadow-lg ${doc.validado ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-blue-400 bg-blue-500/10 border-blue-500/20'}`}>
                      ...
                      <a
                        ...
                        className="w-14 h-14 rounded-2xl bg-[#030712] border border-white/10 text-slate-400 flex items-center justify-center hover:bg-blue-600 hover:text-white hover:border-blue-500 shadow-2xl transition-all duration-500 active:scale-90 group-hover:shadow-blue-500/20"
                      >
...
function QuickLink({ label, href, isSuccess = false }: { label: string, href: string, isSuccess?: boolean }) {
  return (
    <a
      ...
      rel="noopener noreferrer"
      className={`
        flex items-center justify-between p-6 rounded-[2rem] border transition-all duration-500 group/link
        ${isSuccess ? 'bg-emerald-500/10 border-emerald-500/20 hover:bg-emerald-500/20 hover:shadow-[0_0_30px_rgba(16,185,129,0.1)]' : 'bg-white/5 border-white/10 hover:bg-white/10'}
      `}
    >
      <div className="flex items-center gap-4">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-lg border transition-all duration-500 group-hover/link:scale-110 ${isSuccess ? 'bg-emerald-500 text-white border-emerald-400' : 'bg-white/10 text-white border-white/10'}`}>
          <FileText size={18} />
        </div>
      ...
      <ExternalLink size={16} className="opacity-30 group-hover/link:opacity-100 group-hover/link:translate-x-1 transition-all text-blue-400" />
    </a>
  );
}
